...
class FragmentA : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // 이미지 저장 버튼
        val saveImageButton = view.findViewById<FloatingActionButton>(R.id.saveButton)
    ...
    private fun updateLayoutBasedOnImageAspectRatio(uri: Uri, aspectRatio: Float) {
        ...
        val saveImageButton = newView.findViewById<FloatingActionButton>(R.id.saveButton)
        saveImageButton.setOnClickListener {
            saveImage()
        }
    }
    ...
    private fun GCtoggleFab() {
        ...
        val saveButton = requireView().findViewById<FloatingActionButton>(R.id.saveButton)

        val buttonWidth = GCfabMain.width
        val gapInDp = 16 // 간격을 16dp로 지정
        val gapInPx = TypedValue.applyDimension(
        ...
        if (isGCFabOpen) {
            saveButton.animate().translationX(0f)
            ...
            saveButton.animate().translationX((gapInPx+buttonWidth) * 4)
            resendButton.animate().translationX((gapInPx+buttonWidth) * 3)
            cameraButton.animate().translationX((gapInPx+buttonWidth) * 2)
            galleryButton.animate().translationX((gapInPx+buttonWidth) * 1)
    ...
    private fun ColortoggleFab() {
        if (isColorFabOpen) {
            ...
            val buttonHeight = ColorfabMain.height
            val gapInDpP = 24 // 간격을 24dp로 지정
            val gapInPxP = TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP,
                gapInDpP.toFloat(),
                resources.displayMetrics
            )
            val gapInDpL = 16 // 간격을 24dp로 지정
            val gapInPxL = TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP,
                gapInDpL.toFloat(),
                resources.displayMetrics
            )
            ...
            if (PorL) {
                button5.animate().translationY(-(gapInPxP+buttonHeight) * 1)
                button4.animate().translationY(-(gapInPxP+buttonHeight) * 2)
                button3.animate().translationY(-(gapInPxP+buttonHeight) * 3)
                button2.animate().translationY(-(gapInPxP+buttonHeight) * 4)
                button1.animate().translationY(-(gapInPxP+buttonHeight) * 5)
                ColorfabMain.setImageResource(R.drawable.cc_close)
            } else {
                button5.animate().translationY(-(gapInPxP+buttonHeight) * 1)
                button4.animate().translationX(-(gapInPxL+buttonHeight) * 1)
                button3.animate().translationX(-(gapInPxL+buttonHeight) * 2)
                button2.animate().translationX(-(gapInPxL+buttonHeight) * 3)
                button1.animate().translationX(-(gapInPxL+buttonHeight) * 4)
                ColorfabMain.setImageResource(R.drawable.cc_close)
            }
        }
